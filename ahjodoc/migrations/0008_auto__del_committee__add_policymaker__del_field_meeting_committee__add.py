...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'Meeting', fields ['number', 'committee', 'year']
        db.delete_unique(u'ahjodoc_meeting', ['number', 'committee_id', 'year'])

        # Removing unique constraint on 'Meeting', fields ['date', 'committee']
        db.delete_unique(u'ahjodoc_meeting', ['date', 'committee_id'])

        db.rename_column(u'ahjodoc_meeting', 'committee_id', 'policymaker_id')

        # Adding unique constraint on 'Meeting', fields ['date', 'policymaker']
        db.create_unique(u'ahjodoc_meeting', ['date', 'policymaker_id'])

        # Adding unique constraint on 'Meeting', fields ['policymaker', 'number', 'year']
        db.create_unique(u'ahjodoc_meeting', ['policymaker_id', 'number', 'year'])

    def backwards(self, orm):
        # User chose to not deal with backwards NULL issues for 'Meeting.committee'
